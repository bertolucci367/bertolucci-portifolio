...
export default function Products({ products, filters }) {
  ...
  if (pageCount <= 0)
    setPageCount(products.productsConnection.aggregate.count / itemsPerPage);
  ...
  const handlePageClick = (event) => {
    ...
    getAllProductsThumbs(event.selected, itemsPerPage, where).then((data) => {
      ...
      setPageCount(
        Math.ceil(data.productsConnection.aggregate.count / itemsPerPage),
      );
      console.log(data);
    });
  };
  ...
  const handleFilterClick = (event) => {
    ...
    if (item === null || item === 'undefined') {
      console.error('O item enviado está vazio.', item);
    }
  ...
  return (
    <Layout>
      ...
      <Box>
        <Flex>
          <Button
            rightIcon={showFiltersBar ? <AddIcon /> : <MinusIcon />}
      ...
      <Box w="100%">
        <Flex>
          <Box w="200px" pr="20px" hidden={showFiltersBar}>
            <Button
              rightIcon={showTypology ? <AddIcon /> : <MinusIcon />}
              onClick={toggleTypology}
              variant="link"
              color="black"
            >
              <Heading as="h2" size="l" pl="10px" pt="10px" pb="5px">
                TIPOLOGIA{' '}
              </Heading>
            </Button>
            ...
            <Button
              rightIcon={showMaterial ? <AddIcon /> : <MinusIcon />}
            ...
            <Button
              rightIcon={showCategory ? <AddIcon /> : <MinusIcon />}
            ...
            <Button
              rightIcon={showDesigners ? <AddIcon /> : <MinusIcon />}
