...
import { HamburgerIcon, MinusIcon, Search2Icon } from '@chakra-ui/icons';

import Layout from 'src/components/Templates/Layout';

import { getAllProductsThumbs, getAllProductsCount } from 'src/lib/graphcms';
...
export default function Products({ products, productsCount }) {
  ...
  const handlePageClick = (event) => {
    ...
    getAllProductsThumbs(event.selected, productsQtd).then((x) => {
      setCurrentItems(x);
    });
          ...
          <Box w="200px" pr="20px">
            {categorias.map((items, index) => (
              <Box borderBottom="2px solid lightgray" py="10px" key={index}>
                <Heading as="h2" size="l" pl="10px">
                  {items.nome} <MinusIcon pb="3px" />
                </Heading>
                <List p="15px">
                  {items.items.map((x1, y1) => (
                    <ListItem key={y1}>
                      <Checkbox mr="20px" mt="3px"></Checkbox>
                      {x1}
                    </ListItem>
                  ))}
                </List>
