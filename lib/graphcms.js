...
export const getAllProductsThumbs = async ( page, pageCount, where ) => {
  ...
  let query = { 
    ...
    if( where.orderBy.length > 0 ){ query.query.products.__args.orderBy = new EnumType(where.orderBy) }
    
    query.query.productsConnection.__args.where = query.query.products.__args.where;
