...
const fetchCmsAPI = async (query, { variables } = {}) => {
  ...
  const res = await fetch('https://api-us-west-2.graphcms.com/v2/ckhwru7ly4xvt01xs07jqaofg/master', {
    method: 'POST',
    headers: {
...
export const getAllProductsThumbs = async ( page, pageCount, where ) => {
  ...
  pageCount = pageCount
  console.log(`Loading items from ${skiped} to ${skiped + pageCount}`);
  let query = { 
    query: {
      products: {
        __args: {
    ...
    if(where.materials.length > 0){ query.query.products.__args.where.materials_every.id_in = where.materials }
    if(where.lines.length > 0){ query.query.products.__args.where.lines_every.id_in = where.lines }
    if(where.typologies.length > 0){ query.query.products.__args.where.typologies_every.id_in = where.typologies }
    if(where.designer.length > 0){query.query.products.__args.where.designer.id_in = where.designer }
...
export default { getAllDesigners, getAllTypologies, getAllLines, getAllProductsThumbs, getAllProductsCount, getAllFilters };
